...
public class NpcBookFontRenderer extends SmallFontRenderer implements IResourceManagerReloadListener {
	...
	public NpcBookFontRenderer(GameSettings par1GameSettings, ResourceLocation par2ResourceLocation,
		...
		// logger.error(this.locationFontTexture.toString() + " " +
		// this.locationFontTexture.getResourcePath());
	 ...
	 * location...
	 */
	protected float renderDefaultChar(int par1, boolean par2) {
	...
	private void loadGlyphTexture(int par1) {
		// System.out.println("[DEBUG] loadGlyphTexture called with par1 = " + par1);
		ResourceLocation textureLocation = this.getUnicodePageLocation(par1);
		// System.out.println("[DEBUG] Binding texture: " + textureLocation.toString());
		this.renderEngine.bindTexture(textureLocation);
	}
	 ...
	 */
	protected float renderUnicodeChar(char par1, boolean par2) {
	 ...
	 */
	public int drawStringAdvanced(String par1Str, int par2, int par3, int par4) {
		return this.drawStringAdvanced(par1Str, par2, par3, par4, false);
	}

	/**
	 * Draws the specified string. Args: string, x, y, color, dropShadow
	 */
	public int drawStringAdvanced(String par1Str, int par2, int par3, int par4, boolean par5) {
		this.resetStyles();
		...
		if (par5) {
			l = this.renderStringAdvanced(par1Str, par2 + 1, par3 + 1, par4, true);
			l = Math.max(l, this.renderStringAdvanced(par1Str, par2, par3, par4, false));
		} else {
			l = this.renderStringAdvanced(par1Str, par2, par3, par4, false);
		}
	 ...
	 */
	private void renderStringAtPosAdvanced(String par1Str, boolean par2) {
		// System.out.println("DogDog5");
		// this.resetStyles();
			...
			if (c0 == 167 && i + 1 < par1Str.length()) {
				...
				if (this.strikethroughStyle) {
					...
					tessellator.addVertex((double) this.posX, (double) (this.posY + (float) (this.FONT_HEIGHT / 2)),
							0.0D);
					tessellator.addVertex((double) (this.posX + f1),
				...
				if (this.underlineStyle) {
					...
					tessellator.addVertex((double) (this.posX + f1), (double) (this.posY + (float) this.FONT_HEIGHT),
							0.0D);
					tessellator.addVertex((double) (this.posX + f1),
	...
	private int renderStringAligned(String textLine, int x, int y, int par4, int color, boolean darkDropShadow) {
		...
		// System.out.println("DogDog3");

		return this.renderStringAdvanced(textLine, x, y, color, darkDropShadow);
	}
	 ...
	 */
	private int renderStringAdvanced(String par1Str, int x, int y, int color, boolean darkDropShadow) {
		if (par1Str == null) {
			return 0;
		} else {
			if ((color & -67108864) == 0) {
			...
			this.posX = (float) x;
			this.posY = (float) y;
			// System.out.println("DogDog4");
			this.renderStringAtPosAdvanced(par1Str, darkDropShadow);
			if (!gl_blend)
	 ...
	 */
	public void drawSplitStringAdvanced(String par1Str, int par2, int par3, int par4, int par5) {
		this.resetStyles();
		this.textColor = par5;
		par1Str = this.trimStringNewline(par1Str);
		this.renderSplitStringAdvanced(par1Str, par2, par3, par4, false);
		// System.out.println("DogDog1");
	}
	 ...
	 */
	private void renderSplitStringAdvanced(String text, int x, int y, int width, boolean darkDropShadow) {
		List<String> lines = this.listFormattedStringToWidthAdvanced(text, width);
	...
	public int splitStringWidth(String text, int width) {
		return this.FONT_HEIGHT * this.listFormattedStringToWidthAdvanced(text, width).size();
	}
	 ...
	 */
//	public List<String> listFormattedStringToWidthAdvanced(String text, int width) {
//		return Arrays.stream(text.split(NEWLINE_REGEX_SEQUENCE))
//				.flatMap(line -> Arrays.stream(this.wrapFormattedStringToWidthAdvanced(line, width).split("\n")))
...
//	}
	public List<String> listFormattedStringToWidthAdvanced(String text, int width) {
	    ...
	    for (int i = 0; i < chars.length; i++) {
	        ...
	        if (lineWidth >= width) {
	            int breakIndex = BREAK_ITERATOR.preceding(i + 1);
	            if (breakIndex <= lastBreak + 1 || breakIndex == i + 1) {
	                ...
	                lines.add(line.substring(0, relBreak + prevFormatLength));
	 ...
	 * width.
	 */
//	String wrapFormattedStringToWidthAdvanced(String text, int witdh) {
...
//			String s2 = getFormatFromStringAdvanced(s1) + text.substring(j + (flag ? 1 : 0));
//			return s1 + "\n" + this.wrapFormattedStringToWidthAdvanced(s2, witdh);
//		}
//	}
	public String wrapFormattedStringToWidthAdvanced(String text, int width) {
	    if (text.isEmpty()) {
	        return "";
	    }
	    ...
	    for (int i = 0; i < chars.length; i++) {
	        ...
	        if (lineWidth >= width) {
	            ...
	            if (breakIndex <= lastBreak + 1 || breakIndex == i + 1) {
	                ...
	                result.append(line.substring(0, relBreak + prevFormatLength)).append('\n');
	                String rest = line.substring(relBreak + prevFormatLength);
	                String carryFormat = formats[relBreak] != null ? formats[relBreak] : "";
	 ...
	 */
	private static String getFormatFromStringAdvanced(String par0Str) {
	    StringBuilder format = new StringBuilder();
	    int length = par0Str.length();
	    boolean resetEncountered = false;
	    ...
	    // 如果最后是 reset，则下一行开头不应添加任何格式
	    return resetEncountered ? "" : format.toString();
	}

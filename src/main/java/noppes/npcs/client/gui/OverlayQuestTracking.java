...
public class OverlayQuestTracking extends Gui {
	...
	public ArrayList<String> questText = new ArrayList<>();
	public ArrayList<String> questTypeLine = new ArrayList<>();
	...
	public void setOverlayData(NBTTagCompound compound) {
		...
		// questText
		String logText = trackedQuest.getLogText();
		
		if (logText != null && !logText.isEmpty()) {
			int split = logText.indexOf("---");
			if (split != -1) {
				logText = logText.substring(0, split);
			}
		...
		TextBlockClient logTextBlock = new TextBlockClient(logText, (int) (overlayWidth * overallScale * 1.15F), true,
				mc.thePlayer);
		List<String> stringLines = logTextBlock.lines.stream().filter(component -> component != null) // 过滤掉为 null 的组件
				.map(IChatComponent::getFormattedText).map(text -> text != null ? text : "") // 处理可能的 null 字符串
				.collect(Collectors.toList());
		questText.addAll(stringLines);

		// questType
		questTypeLine = getLineList(getLocalizedType(trackedQuest.getType()));
		...
		for (int i = 0; i < nbtTagList.tagCount(); i++) {
			...
			// Translation
			int index = objective.lastIndexOf(":");
			if (index > 0) {
				String name = objective.substring(0, index);
				// Mod. Exp: manametalmod.ModDunageonFire -> 火之领主
				...
				objective = name + objective.substring(index);
			}
			...
			try {
				...
				if (objective.endsWith(StatCollector.translateToLocal("quest.track.done"))
						|| objective.endsWith(StatCollector.translateToLocal("quest.track.read"))
						|| (objective.endsWith(StatCollector.translateToLocal("quest.track.found"))
								&& !objective.endsWith(StatCollector.translateToLocal("quest.track.not_found")))) {
					completed = true;
				}
			}
	...
	private String getLocalizedType(int type) {
		switch (type) {
		case 0:
			return StatCollector.translateToLocal("gui.quesktracking.type.item");
		case 1:
			return StatCollector.translateToLocal("gui.quesktracking.type.dialog");
		case 2:
			return StatCollector.translateToLocal("gui.quesktracking.type.kill");
		case 3:
			return StatCollector.translateToLocal("gui.quesktracking.type.location");
		case 4:
			return StatCollector.translateToLocal("gui.quesktracking.type.areakill");
		case 5:
			return StatCollector.translateToLocal("gui.quesktracking.type.manual");
		default:
			return StatCollector.translateToLocal("gui.quesktracking.type.unknown");
	...
	public ArrayList<String> getLineList(String string) {
		...
		for (int i = 0; i < string.length(); i++) {
			...
			// 获取当前行的内容，加上当前字符后的宽度
			if (this.fontRenderer.getStringWidth(lines.get(lines.size() - 1) + currentChar) > 140) {
				lines.add(currentChar); // 如果超过宽度限制，则新建一行
			} else {
				// 否则，添加到当前行的末尾
				lines.set(lines.size() - 1, lines.get(lines.size() - 1) + currentChar);
			}
		}
	...
	public void renderGameOverlay(float partialTicks) {
		...
		GL11.glPushMatrix();
		GL11.glTranslatef(ConfigClient.TrackingInfoAlignment % 3 * ((float) (scaledWidth) / 2),
				(float) (Math.floor((float) (ConfigClient.TrackingInfoAlignment / 3)) * ((float) (scaledHeight) / 2)),
				0.0F);
		...
		this.renderOffsetY = -10 * overallScale;
		GL11.glPushMatrix();
		this.renderStringLines(questText, 0.9F * overallScale, true, false,
				ConfigClient.TrackingInfoAlignment % 3 == 0 ? 1
						: (ConfigClient.TrackingInfoAlignment % 3 == 2 ? 2 : 0));
		GL11.glPopMatrix();
		...
		GL11.glPushMatrix();
		this.renderStringLines(questTypeLine, 1F * overallScale, true, false,
				ConfigClient.TrackingInfoAlignment % 3 == 0 ? 1
						: (ConfigClient.TrackingInfoAlignment % 3 == 2 ? 2 : 0));
		GL11.glPopMatrix();

		GL11.glPushMatrix();
		this.renderStringLines(objectiveLines, 0.8F * overallScale, true, false,
				ConfigClient.TrackingInfoAlignment % 3 == 0 ? 1
						: (ConfigClient.TrackingInfoAlignment % 3 == 2 ? 2 : 0));
		GL11.glPopMatrix();
		...
		this.renderOffsetY += 10 * overallScale;
		GL11.glPushMatrix();
		this.renderStringLines(turnInText, 0.9F * overallScale, true, false,
				ConfigClient.TrackingInfoAlignment % 3 == 0 ? 1
						: (ConfigClient.TrackingInfoAlignment % 3 == 2 ? 2 : 0));
		GL11.glPopMatrix();
		...
		GL11.glPushMatrix();
		if (overallScale < 0.6) {
			this.drawHorizontalLine((int) (centerX - (overlayWidth / 2 * overallScale)),
					(int) (centerX + (overlayWidth / 2 * overallScale)), (int) ((this.renderOffsetY)), 0xFFFFFFFF);
		} else if (overallScale < 0.8) {
			this.drawHorizontalLine((int) (centerX - (overlayWidth / 2 * overallScale)),
					(int) (centerX + (overlayWidth / 2 * overallScale)), (int) ((this.renderOffsetY)), 0xFFA8A8A8);
			this.drawHorizontalLine((int) (centerX - (overlayWidth / 2 * overallScale)),
					(int) (centerX + (overlayWidth / 2 * overallScale)), (int) ((this.renderOffsetY + 1)), 0xFFFFFFFF);
		} else {
			this.drawHorizontalLine((int) (centerX - (overlayWidth / 2 * overallScale)),
					(int) (centerX + (overlayWidth / 2 * overallScale)), (int) (this.renderOffsetY), 0xFF777777);
			this.drawHorizontalLine((int) (centerX - (overlayWidth / 2 * overallScale)),
					(int) (centerX + (overlayWidth / 2 * overallScale)), (int) ((this.renderOffsetY + 1)), 0xFFA8A8A8);
			this.drawHorizontalLine((int) (centerX - (overlayWidth / 2 * overallScale)),
					(int) (centerX + (overlayWidth / 2 * overallScale)), (int) ((this.renderOffsetY + 2)), 0xFFFFFFFF);
		}
		GL11.glPopMatrix();
	...
	public void renderStringLines(ArrayList<String> lines, float scale, boolean downwards, boolean bold, int facing) {
		for (int i = 0; i < lines.size(); i++) {
			...
			this.fontRenderer.drawStringWithShadow(s, 0, 0, this.color);
			// this.drawString(s, 0, 0, this.color, true);
			if (bold) {
				GL11.glTranslatef(0.2F, 0.2F, 0.0F);
				this.fontRenderer.drawString(s, 0, 0, this.color);
		...
		{
			{
				...
				if (this.strikethroughStyle) {
					...
					tessellator.addVertex((double) this.posX,
							(double) (this.posY + (float) (Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT / 2)),
							0.0D);
					tessellator.addVertex((double) (this.posX + f),
							(double) (this.posY + (float) (Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT / 2)),
							0.0D);
					tessellator
							.addVertex((double) (this.posX + f),
									(double) (this.posY
											+ (float) (Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT / 2) - 1.0F),
									0.0D);
					tessellator
				...
				if (this.underlineStyle) {
					...
					tessellator.addVertex((double) (this.posX + (float) l),
							(double) (this.posY + (float) Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT), 0.0D);
					tessellator.addVertex((double) (this.posX + f),
							(double) (this.posY + (float) Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT), 0.0D);
					tessellator.addVertex((double) (this.posX + f),
							(double) (this.posY + (float) Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT - 1.0F),
							0.0D);
					tessellator.addVertex((double) (this.posX + (float) l),
							(double) (this.posY + (float) Minecraft.getMinecraft().fontRenderer.FONT_HEIGHT - 1.0F),
							0.0D);
	...
	private ResourceLocation getUnicodePageLocation(int p_111271_1_) {
		if (unicodePageLocations[p_111271_1_] == null) {
			unicodePageLocations[p_111271_1_] = new ResourceLocation(String
					.format("textures/font/unicode_page_%02x.png", new Object[] { Integer.valueOf(p_111271_1_) }));
		}
